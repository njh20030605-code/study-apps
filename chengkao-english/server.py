...
PORT_START = 8899          # 首选端口，被占用就依次往后试
PORT_TRIES = 20
KEEP = 30                  # 历史快照最多保留多少份
...
def weight(state):
    ...
    ——空备份不许覆盖非空备份。两套 App 的字段不同，都认。"""
    try:
        s = state or {}
        p = s.get("progress") or {}
        old = (len(p.get("srs") or {}), int(p.get("totalAnswered") or 0))
        new = (len(s.get("log") or []), len(s.get("vocab") or []))
        return old if old != (0, 0) else new
    except Exception:
        return (0, 0)
...
class Handler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            ...
            if weight(state) == (0, 0) and os.path.exists(LATEST):
                try:
                    with open(LATEST, "r", encoding="utf-8") as f:
                        if weight(json.load(f)) != (0, 0):
